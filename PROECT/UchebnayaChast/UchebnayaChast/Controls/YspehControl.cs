...
namespace UchebnayaChast
{
    public partial class YspehControl : UserControl
    {
        ...
        public void SrBall()
        {
            ...
            LabelVsegoOcenok.Text = kolvo.ToString();
        ...
        {
            OpenForm(new FormPrint(YspehGrid));
        }

        private void BtnPrint2_Click(object sender, EventArgs e)
        {
            OpenForm(new FormPrint(YspehGrid, false));
        }

        private void BtnFncChange_Click(object sender, EventArgs e)
        {
            if (Functional.RowTest(YspehGrid))
            {
                OpenForm(new FormAddChangeYspeh(Functional.Controller.TakeRowWithNamesById(SpecialSqlController.Tables.yspeh, int.Parse(Functional.GetId(YspehGrid)))));
                Actions();
            }
        }
        ...
        {
            if (Functional.RowTest(YspehGrid, false))
            {
                    Functional.Delete(YspehGrid, SpecialSqlController.Tables.yspeh);
                    Actions();
            }
        }

        private void BtnCheckYspehKicked_Click(object sender, EventArgs e)
        {
            OpenForm(new FormPrintYspehKicked());
        }
